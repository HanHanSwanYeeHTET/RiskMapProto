...
void SpawnTurtles() {
    for (int i = 0; i < numberOfTurtles; i++) {
        ...
        // Ensure we're not spawning a turtle in water.
        // If the chosen cell is water, keep searching until we find land.
        while (grid[x, y].isWater) {
            x = Random.Range(0, size);
            y = Random.Range(0, size);
        }

        // Instantiate the turtle prefab at the chosen location.
        Vector3 spawnPosition = new Vector3(x, 0, y); // Adjust y as needed based on your terrain height
